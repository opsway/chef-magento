...
execute "Reset support admin password" do
	command <<-EOF
		/usr/local/bin/n98-magerun.phar --root-dir=#{node['magento']['docroot']} admin:user:create #{node['magento']['supportuser']['username']} #{node['magento']['supportuser']['email']} #{node['magento']['supportuser']['password']} #{random_password} Support Admin #{node['magento']['supportuser']['role']}
